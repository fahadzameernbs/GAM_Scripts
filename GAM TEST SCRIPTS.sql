...
--GAM COUNT
SELECT COUNT(*) FROM curated_integration.global_account_master; -- 95763936
--SG_GAM COUNT
SELECT COUNT(*) FROM surrogate.sg_global_account_master; -- 96063767
...
-- MDMS
SELECT COUNT(*)
FROM atomic.dwv41141_account_dtl WHERE business_entity_code <> ''; -- 20090504
...
AND TRIM(legacy.distb_cntry) <> '';  -- 71942249

-- ATLAS
SELECT COUNT(*)
FROM atomic_legacy.dwt40016_imc atlas
  JOIN PUBLIC.temp_dwt12011_qa tmp ON atlas.home_country = tmp.iso_cntry_cd
  LEFT JOIN (SELECT distb.intgrt_ctl_aff,
                    distb.distb_cntry,
                    distb.distb_nbr
             FROM atomic_legacy.dwt41042_distb_dtl distb
               JOIN atomic.wwt01020_aff_mst am_valid_aff ON am_valid_aff.aff_id = CASE WHEN TRIM (distb.intgrt_ctl_aff) = '' THEN 0 ELSE distb.intgrt_ctl_aff::SMALLINT END) legacy ON atlas.imc_number = CONCAT (LPAD (legacy.intgrt_ctl_aff,3,'0'),legacy.distb_cntry)
  LEFT JOIN atomic.dwv41141_account_dtl mdms ON atlas.imc_number = CONCAT (LPAD (mdms.affiliate_code,3,'0'),mdms.abo_no)
WHERE (mdms.affiliate_code IS NULL OR TRIM(mdms.business_entity_code) = '')
AND   (legacy.intgrt_ctl_aff IS NULL OR TRIM(legacy.distb_cntry) = '')
AND   TRIM(atlas.imc_type) <> 'INTERCOMPANY'
AND   TRIM(tmp.amway_cntry_cd) <> ''; -- 1116105

--TOTAL
SELECT 20090504 + 71942249 + 1116105; -- 93148858
...
WHERE mdms.business_entity_code <> '';  --20090504

--LEGACY
SELECT COUNT(*)
FROM curated_integration.global_account_master gam
  JOIN atomic_legacy.dwt41042_distb_dtl legacy ON gam.global_account_id = CONCAT( LPAD(legacy.intgrt_ctl_aff,'3','0'),legacy.distb_nbr)
  JOIN atomic.wwt01020_aff_mst am_valid_aff
		ON am_valid_aff.aff_id = CASE WHEN TRIM(legacy.intgrt_ctl_aff) = '' THEN 0	ELSE legacy.intgrt_ctl_aff::SMALLINT END
	LEFT JOIN atomic.dwv41141_account_dtl mdms ON CONCAT ( LPAD( legacy.intgrt_ctl_aff,'3','0'),legacy.distb_nbr) = CONCAT (LPAD( mdms.affiliate_code,'3','0'),mdms.abo_no)
WHERE (mdms.affiliate_code IS NULL OR mdms.business_entity_code = '')
AND   TRIM(legacy.distb_cntry) <> '';  --71942243
...
FROM curated_integration.global_account_master gam
  ...
  LEFT JOIN (SELECT distb.intgrt_ctl_aff,
                    ...
                    distb.distb_nbr
             FROM atomic_legacy.dwt41042_distb_dtl distb
               JOIN atomic.wwt01020_aff_mst am_valid_aff ON am_valid_aff.aff_id = CASE WHEN TRIM (distb.intgrt_ctl_aff) = '' THEN 0 ELSE distb.intgrt_ctl_aff::SMALLINT END) legacy ON atlas.imc_number = CONCAT (LPAD (legacy.intgrt_ctl_aff,3,'0'),legacy.distb_nbr)
  LEFT JOIN atomic.dwv41141_account_dtl mdms ON atlas.imc_number = CONCAT (LPAD (mdms.affiliate_code,3,'0'),mdms.abo_no)
WHERE (mdms.affiliate_code IS NULL OR TRIM(mdms.business_entity_code) = '')
AND   (legacy.intgrt_ctl_aff IS NULL OR TRIM(legacy.distb_cntry) = '')
AND   TRIM(atlas.imc_type) <> 'INTERCOMPANY'
...
--TOTAL
SELECT 20090504 + 71942243 + 1116105; -- 93148852
...
SELECT LPAD(mdms.affiliate_code,'3','0'),
       mdms.abo_no,
       COUNT(*)
FROM atomic.dwv41141_account_dtl mdms
WHERE mdms.business_entity_code <> ''
GROUP BY 1,2
HAVING COUNT(*) > 1 LIMIT 10;  -- 0

SELECT COUNT(*)
FROM atomic.dwv41141_account_dtl
WHERE length(trim(affiliate_code))<3;  --0

--LEGACY
SELECT LPAD(legacy.intgrt_ctl_aff,'3','0'),
       legacy.distb_nbr,
       COUNT(*)
FROM atomic_legacy.dwt41042_distb_dtl legacy
WHERE legacy.distb_cntry <> ''
GROUP BY 1,2
HAVING COUNT(*) > 1 LIMIT 10;  --0 

SELECT COUNT(*)
FROM atomic_legacy.dwt41042_distb_dtl
WHERE length(trim(intgrt_ctl_aff))<3;  --0
...
GROUP BY 1
HAVING COUNT(*) > 1 LIMIT 100; -- 0

SELECT abo_number,affiliate_code,
       COUNT(*)
FROM curated_integration.global_account_master
GROUP BY 1,2
HAVING COUNT(*) > 1 LIMIT 100; -- 0
...
FROM atomic_legacy.dwt41042_distb_dtl legacy
  JOIN atomic.wwt01020_aff_mst am_valid_aff
		ON am_valid_aff.aff_id = CASE WHEN TRIM(legacy.intgrt_ctl_aff) = '' THEN 0	ELSE legacy.intgrt_ctl_aff::SMALLINT END
	JOIN surrogate.sg_global_account_master sg_gam
    ON legacy.distb_nbr = sg_gam.abo_number
   AND LPAD (legacy.intgrt_ctl_aff,'3','0') = sg_gam.affiliate_code
   LEFT JOIN atomic.dwv41141_account_dtl mdms ON CONCAT (LPAD( legacy.intgrt_ctl_aff,'3','0'), legacy.distb_nbr) = CONCAT (LPAD( mdms.affiliate_code,'3','0'), mdms.abo_no)
WHERE (mdms.affiliate_code IS NULL OR TRIM(mdms.business_entity_code) = '')
AND   legacy.distb_cntry <> '';  -- 71942243
...
SELECT COUNT(*) FROM surrogate.sg_global_account_master sgam
    ...
    JOIN PUBLIC.temp_dwt12011_qa tmp ON atlas.home_country = tmp.iso_cntry_cd
    LEFT JOIN (SELECT distb.intgrt_ctl_aff, distb.distb_cntry, distb.distb_nbr
                FROM atomic_legacy.dwt41042_distb_dtl distb
                JOIN atomic.wwt01020_aff_mst am_valid_aff
        ON am_valid_aff.aff_id = CASE WHEN TRIM(distb.intgrt_ctl_aff) = '' THEN 0    ELSE distb.intgrt_ctl_aff::SMALLINT END
    ) legacy
        ON atlas.imc_number = CONCAT(LPAD( legacy.intgrt_ctl_aff,3,'0'),legacy.distb_nbr)
    LEFT JOIN atomic.dwv41141_account_dtl mdms
        ON atlas.imc_number = CONCAT(LPAD( mdms.affiliate_code,3,'0'),mdms.abo_no)
    WHERE (mdms.affiliate_code IS NULL OR TRIM(mdms.business_entity_code) = '')
        AND (legacy.intgrt_ctl_aff IS NULL OR TRIM(legacy.distb_cntry) = '')
        AND TRIM(atlas.imc_type) <> 'INTERCOMPANY' AND TRIM(tmp.amway_cntry_cd) <> '';  --1116105
...
--MUST BE A NUMBER (QR)
SELECT COUNT(*)
FROM curated_integration.global_account_master gam WHERE gam.global_account_wid ~ '^[0-9]+$' = 'False';  --91486280
...
FROM curated_integration.global_account_master gam
  ...
  WHERE sg_gam.global_account_wid = gam.global_account_wid; -- 95763936
  
SELECT COUNT(*)
FROM curated_integration.global_account_master gam   --95065586
  JOIN surrogate.sg_global_account_master sg_gam ON gam.global_account_id = CONCAT(sg_gam.affiliate_code,sg_gam.abo_number) 
  WHERE sg_gam.global_account_wid = gam.global_account_wid; -- 95763928
  
-- 2. global_account_id

--MUST BE A NUMBER (QR)
SELECT *
FROM curated_integration.global_account_master gam WHERE gam.global_account_id ~ '^[0-9]+$' = 'FALSE' limit 10;  --91486521
...
SELECT COUNT(*)
FROM curated_integration.global_account_master gam
WHERE gam.account_id ~ '^[0-9]+$' = 'FALSE';  --91486527
...
FROM curated_integration.global_account_master gam
  JOIN atomic.dwv41141_account_dtl mdms ON gam.global_account_id = CONCAT (LPAD( mdms.affiliate_code,'3','0'),mdms.abo_no)
WHERE mdms.business_entity_code <> '' AND gam.account_id = mdms.account_id;  -- 20090504
...
FROM curated_integration.global_account_master gam
  ...
  JOIN atomic.wwt01020_aff_mst am_valid_aff
	...
	LEFT JOIN atomic.dwv41141_account_dtl mdms ON CONCAT (LPAD(legacy.intgrt_ctl_aff,'3','0'),legacy.distb_nbr) = CONCAT (LPAD(mdms.affiliate_code,'3','0'),mdms.abo_no)
WHERE (CONCAT(mdms.affiliate_code,mdms.abo_no) IS NULL OR TRIM(mdms.business_entity_code) = '')
AND   legacy.distb_cntry <> '' AND gam.account_id = legacy.distb_nbr GROUP BY 2 HAVING (COUNT(*)) > 1 LIMIT 10;  --FOUND DUPLICATES
...
FROM curated_integration.global_account_master gam
  ...
  JOIN atomic.wwt01020_aff_mst am_valid_aff
	...
	LEFT JOIN atomic.dwv41141_account_dtl mdms ON CONCAT (LPAD(legacy.intgrt_ctl_aff,'3','0'),legacy.distb_nbr) = CONCAT (LPAD(mdms.affiliate_code,'3','0'),mdms.abo_no)
WHERE (CONCAT(mdms.affiliate_code,mdms.abo_no) IS NULL OR TRIM(mdms.business_entity_code) = '')
AND   legacy.distb_cntry <> '' AND gam.account_id = legacy.distb_nbr; -- 71942243
...
AND   (legacy.intgrt_ctl_aff IS NULL OR TRIM(legacy.distb_cntry) = '')
AND   TRIM(atlas.imc_type) <> 'INTERCOMPANY'
AND   TRIM(tmp.amway_cntry_cd) <> '' AND gam.account_id = SUBSTRING(atlas.imc_number,4)::BIGINT; -- 1116105
...
FROM curated_integration.global_account_master gam
  JOIN atomic.dwv41141_account_dtl mdms ON gam.global_account_id = CONCAT (LPAD( mdms.affiliate_code,'3','0'),mdms.abo_no)
WHERE mdms.business_entity_code <> '' AND gam.abo_number = mdms.abo_no;  -- 20090504
...
FROM curated_integration.global_account_master gam
  ...
  JOIN atomic.wwt01020_aff_mst am_valid_aff
	...
	LEFT JOIN atomic.dwv41141_account_dtl mdms ON CONCAT (LPAD(legacy.intgrt_ctl_aff,'3','0'),legacy.distb_nbr) = CONCAT (LPAD(mdms.affiliate_code,'3','0'),mdms.abo_no)
WHERE (CONCAT(mdms.affiliate_code,mdms.abo_no) IS NULL OR TRIM(mdms.business_entity_code) = '')
AND   legacy.distb_cntry <> '' AND gam.abo_number = legacy.distb_nbr
GROUP BY 2 HAVING(COUNT(*)) > 1 LIMIT 20;   --FOUND DUPLICATES
...
FROM curated_integration.global_account_master gam
  ...
  JOIN atomic.wwt01020_aff_mst am_valid_aff
	...
	LEFT JOIN atomic.dwv41141_account_dtl mdms ON CONCAT (LPAD(legacy.intgrt_ctl_aff,'3','0'),legacy.distb_nbr) = CONCAT (LPAD(mdms.affiliate_code,'3','0'),mdms.abo_no)
WHERE (CONCAT(mdms.affiliate_code,mdms.abo_no) IS NULL OR TRIM(mdms.business_entity_code) = '')
AND   legacy.distb_cntry <> '' AND gam.abo_number = legacy.distb_nbr; -- 71942243
...
AND   (legacy.intgrt_ctl_aff IS NULL OR TRIM(legacy.distb_cntry) = '')
AND   TRIM(atlas.imc_type) <> 'INTERCOMPANY'
AND   TRIM(tmp.amway_cntry_cd) <> '' AND gam.abo_number = SUBSTRING(atlas.imc_number,4)::BIGINT; -- 1116105
...
FROM curated_integration.global_account_master gam
  JOIN atomic.dwv41141_account_dtl mdms ON gam.global_account_id = CONCAT (LPAD( mdms.affiliate_code,'3','0'),mdms.abo_no)
WHERE mdms.business_entity_code <> '' AND gam.affiliate_code = LPAD(mdms.affiliate_code,'3','0');  -- 20090504
...
AND   legacy.distb_cntry <> '' AND gam.affiliate_code = LPAD(legacy.intgrt_ctl_aff,'3','0'); -- 71942243
...
AND   (legacy.intgrt_ctl_aff IS NULL OR TRIM(legacy.distb_cntry) = '')
AND   TRIM(atlas.imc_type) <> 'INTERCOMPANY'
AND   TRIM(tmp.amway_cntry_cd) <> '' AND gam.affiliate_code = NVL(atlas.intgrt_aff_cd, atlas.sales_plan_affiliate); -- 1116105
...
WHERE mdms.business_entity_code <> ''
AND   gam.affiliate_name = ndt_aff_cntry_mst.aff_desc;  -- 20090504
...
WHERE (CONCAT(mdms.affiliate_code,mdms.abo_no) IS NULL OR mdms.business_entity_code = '')
AND   legacy.distb_cntry <> '' AND gam.affiliate_name = ndt_aff_cntry_mst.aff_desc; -- 71942243
...
AND   (legacy.intgrt_ctl_aff IS NULL OR TRIM(legacy.distb_cntry) = '')
AND   TRIM(atlas.imc_type) <> 'INTERCOMPANY'
AND   TRIM(tmp.amway_cntry_cd) <> '' AND gam.affiliate_name = ndt_aff_cntry_mst.aff_desc; -- 1116105
...
FROM curated_integration.global_account_master gam
  JOIN atomic.dwv41141_account_dtl mdms ON gam.global_account_id = CONCAT (LPAD (mdms.affiliate_code,'3','0'),mdms.abo_no)
WHERE mdms.business_entity_code <> ''
AND   gam.business_entity_code = LPAD(mdms.business_entity_code,'3','0');  --20090504
...
FROM curated_integration.global_account_master gam
  ...
  JOIN atomic.wwt01020_aff_mst am_valid_aff
	...
	LEFT JOIN atomic.dwv41141_account_dtl mdms ON CONCAT (LPAD(legacy.intgrt_ctl_aff,'3','0'),legacy.distb_nbr) = CONCAT (LPAD(mdms.affiliate_code,'3','0'),mdms.abo_no)
WHERE (mdms.affiliate_code IS NULL OR TRIM(mdms.business_entity_code) = '')
AND   legacy.distb_cntry <> '' AND gam.business_entity_code = legacy.distb_cntry; --71942195
...
AND   (legacy.intgrt_ctl_aff IS NULL OR TRIM(legacy.distb_cntry) = '')
AND   TRIM(atlas.imc_type) <> 'INTERCOMPANY'
AND   TRIM(tmp.amway_cntry_cd) <> '' AND gam.business_entity_code = tmp.amway_cntry_cd;  --1116105
...
FROM curated_integration.global_account_master gam
  ...
  JOIN atomic.wwt01020_aff_mst am_valid_aff
		ON am_valid_aff.aff_id = CASE WHEN TRIM(legacy.intgrt_ctl_aff) = '' THEN 0	ELSE legacy.intgrt_ctl_aff::SMALLINT END
	LEFT JOIN atomic.dwv41141_account_dtl mdms ON CONCAT (LPAD(legacy.intgrt_ctl_aff,'3','0'),legacy.distb_nbr) = CONCAT (LPAD(mdms.affiliate_code,'3','0'),mdms.abo_no)
  JOIN public.dwt41036_ddatbat_qa ndt_ddatbat_qa ON ndt_ddatbat_qa.intgrt_ctl_aff = LPAD(legacy.intgrt_ctl_aff,'3','0') AND ndt_ddatbat_qa.distb_nbr = legacy.distb_nbr
WHERE (mdms.affiliate_code IS NULL OR TRIM(mdms.business_entity_code) = '')
AND   legacy.distb_cntry <> ''
...
FROM curated_integration.global_account_master gam
  ...
  JOIN atomic.wwt01020_aff_mst am_valid_aff
		ON am_valid_aff.aff_id = CASE WHEN TRIM(legacy.intgrt_ctl_aff) = '' THEN 0	ELSE legacy.intgrt_ctl_aff::SMALLINT END
	LEFT JOIN atomic.dwv41141_account_dtl mdms ON CONCAT (LPAD(legacy.intgrt_ctl_aff,'3','0'),legacy.distb_nbr) = CONCAT (LPAD(mdms.affiliate_code,'3','0'),mdms.abo_no)
  LEFT JOIN public.dwt41036_ddatbat_qa ndt_ddatbat_qa ON ndt_ddatbat_qa.intgrt_ctl_aff = LPAD(legacy.intgrt_ctl_aff,'3','0') AND ndt_ddatbat_qa.distb_nbr = legacy.distb_nbr
WHERE (mdms.affiliate_code IS NULL OR TRIM(mdms.business_entity_code) = '')
AND   legacy.distb_cntry <> ''
...
FROM curated_integration.global_account_master gam
  ...
  ON LPAD(legacy.intgrt_ctl_aff,3,'0') <> '010'
  AND gam.global_account_id = CONCAT( LPAD(legacy.intgrt_ctl_aff,'3','0'), legacy.distb_nbr)
  JOIN atomic.wwt01020_aff_mst am_valid_aff
		ON am_valid_aff.aff_id = CASE WHEN TRIM(legacy.intgrt_ctl_aff) = '' THEN 0	ELSE legacy.intgrt_ctl_aff::SMALLINT END
	LEFT JOIN atomic.dwv41141_account_dtl mdms ON CONCAT (LPAD(legacy.intgrt_ctl_aff,'3','0'),legacy.distb_nbr) = CONCAT (LPAD(mdms.affiliate_code,'3','0'),mdms.abo_no)
WHERE (mdms.affiliate_code IS NULL OR TRIM(mdms.business_entity_code) = '')
...
FROM curated_integration.global_account_master gam
  ...
  JOIN atomic.wwt01020_aff_mst am_valid_aff
	...
	ON lpad(legacy.intgrt_ctl_aff, 3, '0') = '010'
        AND LPAD(ndt_dimcprf.intgrt_ctl_aff, 3, '0') = lpad(legacy.intgrt_ctl_aff, 3, '0')
        AND ndt_dimcprf.distb_nbr = legacy.distb_nbr
...
FROM curated_integration.global_account_master gam
  ...
  JOIN atomic.wwt01020_aff_mst am_valid_aff
		ON am_valid_aff.aff_id = CASE WHEN TRIM(legacy.intgrt_ctl_aff) = '' THEN 0	ELSE legacy.intgrt_ctl_aff::SMALLINT END
	LEFT JOIN atomic.dwv41141_account_dtl mdms ON CONCAT (LPAD(legacy.intgrt_ctl_aff,'3','0'),legacy.distb_nbr) = CONCAT (LPAD(mdms.affiliate_code,'3','0'),mdms.abo_no)
	JOIN atomic_legacy.dwt41047_ddsttyp ndt_ddsttyp ON legacy.imc_type_code = ndt_ddsttyp.dist_type AND legacy.distb_cntry = ndt_ddsttyp.intgrt_cntry_cd
WHERE (mdms.affiliate_code IS NULL OR TRIM(mdms.business_entity_code) = '')
AND   legacy.distb_cntry <> ''
...
FROM curated_integration.global_account_master gam
  ...
  JOIN atomic.wwt01020_aff_mst am_valid_aff
		ON am_valid_aff.aff_id = CASE WHEN TRIM(legacy.intgrt_ctl_aff) = '' THEN 0	ELSE legacy.intgrt_ctl_aff::SMALLINT END
	LEFT JOIN atomic.dwv41141_account_dtl mdms ON CONCAT (LPAD(legacy.intgrt_ctl_aff,'3','0'),legacy.distb_nbr) = CONCAT (LPAD(mdms.affiliate_code,'3','0'),mdms.abo_no)
	JOIN atomic_legacy.dwt41046_dbusnat ndt_dbusnat ON legacy.business_nature_code = ndt_dbusnat.bus_nat_cd AND legacy.distb_cntry = ndt_dbusnat.intgrt_cntry_cd
WHERE (mdms.affiliate_code IS NULL OR TRIM(mdms.business_entity_code) = '')
AND   legacy.distb_cntry <> ''
...
FROM curated_integration.global_account_master gam
  ...
  JOIN atomic.wwt01020_aff_mst am_valid_aff
		ON am_valid_aff.aff_id = CASE WHEN TRIM(legacy.intgrt_ctl_aff) = '' THEN 0	ELSE legacy.intgrt_ctl_aff::SMALLINT END
	LEFT JOIN atomic.dwv41141_account_dtl mdms ON CONCAT (LPAD(legacy.intgrt_ctl_aff,'3','0'),legacy.distb_nbr) = CONCAT (LPAD(mdms.affiliate_code,'3','0'),mdms.abo_no)
	LEFT JOIN atomic_legacy.dwt41046_dbusnat ndt_dbusnat ON legacy.business_nature_code = ndt_dbusnat.bus_nat_cd AND legacy.distb_cntry = ndt_dbusnat.intgrt_cntry_cd
WHERE (mdms.affiliate_code IS NULL OR TRIM(mdms.business_entity_code) = '')
AND   legacy.distb_cntry <> ''
...
FROM curated_integration.global_account_master gam
  ...
  JOIN atomic.wwt01020_aff_mst am_valid_aff
    ...
    ON legacy.distb_cntry = seg.Cntry_Cd
...
FROM curated_integration.global_account_master gam
  ...
  JOIN atomic.wwt01020_aff_mst am_valid_aff
		ON am_valid_aff.aff_id = CASE WHEN TRIM(legacy.intgrt_ctl_aff) = '' THEN 0	ELSE legacy.intgrt_ctl_aff::SMALLINT END
	LEFT JOIN atomic.dwv41141_account_dtl mdms ON CONCAT (LPAD(legacy.intgrt_ctl_aff,'3','0'),legacy.distb_nbr) = CONCAT (LPAD(mdms.affiliate_code,'3','0'),mdms.abo_no)
	JOIN atomic.wwt01080_ibo_inf_mst ndt_ibo_inf_mst ON LPAD (legacy.intgrt_ctl_aff,3,'0') = LPAD (ndt_ibo_inf_mst.aff_id,3,'0') AND legacy.distb_nbr = ndt_ibo_inf_mst.ibo_no
  JOIN surrogate.sg_global_account_master sg_gam ON sg_gam.affiliate_code = gam.affiliate_code AND sg_gam.abo_number = ndt_ibo_inf_mst.spon_ibo_no
WHERE (mdms.affiliate_code IS NULL OR TRIM(mdms.business_entity_code) = '')
AND   legacy.distb_cntry <> ''
...
FROM curated_integration.global_account_master gam
  ...
  JOIN atomic.wwt01020_aff_mst am_valid_aff
	...
	JOIN atomic.wwt01080_ibo_inf_mst ndt_ibo_inf_mst ON LPAD (legacy.intgrt_ctl_aff,3,'0') = LPAD (ndt_ibo_inf_mst.aff_id,3,'0') AND legacy.distb_nbr = ndt_ibo_inf_mst.ibo_no
WHERE (mdms.affiliate_code IS NULL OR TRIM(mdms.business_entity_code) = '')
AND   legacy.distb_cntry <> ''  --74340525
...
FROM curated_integration.global_account_master gam
  ...
  JOIN atomic.wwt01020_aff_mst am_valid_aff
		ON am_valid_aff.aff_id = CASE WHEN TRIM(legacy.intgrt_ctl_aff) = '' THEN 0	ELSE legacy.intgrt_ctl_aff::SMALLINT END
	LEFT JOIN atomic.dwv41141_account_dtl mdms ON CONCAT (LPAD(legacy.intgrt_ctl_aff,'3','0'),legacy.distb_nbr) = CONCAT (LPAD(mdms.affiliate_code,'3','0'),mdms.abo_no)
	LEFT JOIN atomic.wwt01080_ibo_inf_mst ndt_ibo_inf_mst ON LPAD (legacy.intgrt_ctl_aff,3,'0') = LPAD (ndt_ibo_inf_mst.aff_id,3,'0') AND legacy.distb_nbr = ndt_ibo_inf_mst.ibo_no
WHERE (mdms.affiliate_code IS NULL OR TRIM(mdms.business_entity_code) = '')
AND   legacy.distb_cntry <> ''
...
FROM curated_integration.global_account_master gam
  JOIN atomic.dwv41141_account_dtl mdms ON gam.global_account_id = CONCAT (LPAD (mdms.affiliate_code,'3','0'),mdms.abo_no)
WHERE gam.source_name = 'dwv41141_account_dtl';  --1788918
...
FROM curated_integration.global_account_master gam
  JOIN atomic.dwv41141_account_dtl mdms ON gam.global_account_id = CONCAT (LPAD (mdms.affiliate_code,'3','0'),mdms.abo_no)
  JOIN atomic.wwt01080_ibo_inf_mst ndt_ibo_inf_mst
    ON    LPAD (mdms.affiliate_code,3,'0') = LPAD( ndt_ibo_inf_mst.aff_id,3,'0')
    AND   mdms.abo_no = ndt_ibo_inf_mst.ibo_no
WHERE mdms.business_entity_code <> ''
AND   gam.application_entry_date = ndt_ibo_inf_mst.app_entry_dt::DATE;  --8956266

SELECT COUNT(*)
FROM curated_integration.global_account_master gam
  JOIN atomic.dwv41141_account_dtl mdms ON gam.global_account_id = CONCAT (LPAD (mdms.affiliate_code,'3','0'),mdms.abo_no)
  LEFT JOIN atomic.wwt01080_ibo_inf_mst ndt_ibo_inf_mst
    ON    LPAD (mdms.affiliate_code,3,'0') = LPAD( ndt_ibo_inf_mst.aff_id,3,'0')
    AND   mdms.abo_no = ndt_ibo_inf_mst.ibo_no
WHERE mdms.business_entity_code <> ''
AND   ndt_ibo_inf_mst.aff_id IS NULL
AND   gam.application_entry_date = mdms.entry_date::DATE;  --4768

SELECT 8956266 + 4768;  --8961034

--LEGACY   82199777
SELECT COUNT(*)
FROM curated_integration.global_account_master gam
  JOIN atomic_legacy.dwt41042_distb_dtl legacy ON gam.global_account_id = CONCAT (LPAD (legacy.intgrt_ctl_aff,'3','0'),legacy.distb_nbr)
  JOIN atomic.wwt01020_aff_mst am_valid_aff ON am_valid_aff.aff_id = CASE WHEN TRIM (legacy.intgrt_ctl_aff) = '' THEN 0 ELSE legacy.intgrt_ctl_aff::SMALLINT END
  LEFT JOIN atomic.dwv41141_account_dtl mdms ON CONCAT (LPAD (legacy.intgrt_ctl_aff,'3','0'),legacy.distb_nbr) = CONCAT (LPAD (mdms.affiliate_code,'3','0'),mdms.abo_no)
  JOIN atomic.wwt01080_ibo_inf_mst ndt_ibo_inf_mst
    ON    LPAD (legacy.intgrt_ctl_aff,3,'0') = LPAD( ndt_ibo_inf_mst.aff_id,3,'0')
    AND   legacy.distb_nbr = ndt_ibo_inf_mst.ibo_no
WHERE (mdms.affiliate_code IS NULL OR TRIM(mdms.business_entity_code) = '')
AND   legacy.distb_cntry <> ''
AND   gam.application_entry_date = ndt_ibo_inf_mst.app_entry_dt::DATE;  --66880157
...
AND   legacy.distb_cntry <> ''
AND   ndt_ibo_inf_mst.aff_id IS NULL
AND   legacy.signed_contract_date IS NOT NULL
AND   TRIM(legacy.signed_contract_date) <> ''
AND   legacy.signed_contract_date <> '19000000'
AND   gam.application_entry_date = legacy.signed_contract_date::DATE;  --12542079
...
AND   legacy.distb_cntry <> ''
AND   ndt_ibo_inf_mst.aff_id IS NULL
AND   (legacy.signed_contract_date IS NULL OR TRIM(legacy.signed_contract_date) = '' OR legacy.signed_contract_date = '19000000')
AND   gam.application_entry_date = '19000101'::DATE;  --2777541

SELECT 66880157 + 12542079 + 2777541;  --82199777
...
FROM curated_integration.global_account_master gam
  ...
  LEFT JOIN atomic.dwv41141_account_dtl mdms ON atlas.imc_number = CONCAT (LPAD (mdms.affiliate_code,3,'0'),mdms.abo_no)
  JOIN atomic.wwt01080_ibo_inf_mst ndt_ibo_inf_mst
    ON    atlas.imc_number = CONCAT( LPAD( ndt_ibo_inf_mst.aff_id,3,'0'), ndt_ibo_inf_mst.ibo_no)
WHERE (mdms.affiliate_code IS NULL OR TRIM(mdms.business_entity_code) = '')
AND   (legacy.intgrt_ctl_aff IS NULL OR TRIM(legacy.distb_cntry) = '')
AND   TRIM(atlas.imc_type) <> 'INTERCOMPANY'
AND   TRIM(tmp.amway_cntry_cd) <> ''
AND   gam.application_entry_date = ndt_ibo_inf_mst.app_entry_dt::DATE; --1102148
...
AND   TRIM(tmp.amway_cntry_cd) <> ''
AND   ndt_ibo_inf_mst.aff_id IS NULL
AND   atlas.application_date IS NOT NULL
AND   atlas.application_date <> ''
AND   atlas.application_date <> '19000000'
AND   gam.application_entry_date = atlas.application_date::DATE;  --6813
...
AND   TRIM(tmp.amway_cntry_cd) <> ''
AND   ndt_ibo_inf_mst.aff_id IS NULL
AND   (atlas.application_date IS NULL OR TRIM(atlas.application_date) = '' OR atlas.application_date = '19000000')
AND   gam.application_entry_date = '19000101'::DATE;  --1596

SELECT 1102148 + 6813 + 1596;  --1110557
...
FROM curated_integration.global_account_master gam
  ...
  JOIN atomic.wwt01020_aff_mst am_valid_aff
	...
	LEFT JOIN atomic.dwv41141_account_dtl mdms ON CONCAT (LPAD(legacy.intgrt_ctl_aff,'3','0'),legacy.distb_nbr) = CONCAT (LPAD(mdms.affiliate_code,'3','0'),mdms.abo_no)
  JOIN atomic_legacy.dwt41150_dist_type_legacy_lkp ndt_dist_type_legacy_lkp
    ON LPAD( legacy.intgrt_ctl_aff,3,'0')::SMALLINT = ndt_dist_type_legacy_lkp.aff_no
...
FROM curated_integration.global_account_master gam
  ...
  JOIN atomic.wwt01020_aff_mst am_valid_aff
    ...
    ON lpad(lcl_imc_type.aff_id, 3, '0') = lpad(legacy.intgrt_ctl_aff, 3, '0')
...
FROM curated_integration.global_account_master gam
  ...
  JOIN atomic.wwt01020_aff_mst am_valid_aff
	...
	JOIN atomic_legacy.dwt41047_ddsttyp ndt_ddsttyp ON legacy.imc_type_code = ndt_ddsttyp.dist_type AND legacy.distb_cntry = ndt_ddsttyp.intgrt_cntry_cd
WHERE (mdms.affiliate_code IS NULL OR TRIM(mdms.business_entity_code) = '')
AND   legacy.distb_cntry <> '' AND gam.original_imc_type_desc = ndt_ddsttyp.dist_type_desc; -- 71710621
...
FROM curated_integration.global_account_master gam
  ...
  JOIN atomic.wwt01020_aff_mst am_valid_aff
	...
	JOIN atomic_legacy.dwt41047_ddsttyp ndt_ddsttyp ON legacy.imc_type_code = ndt_ddsttyp.dist_type AND legacy.distb_cntry = ndt_ddsttyp.intgrt_cntry_cd
WHERE (mdms.affiliate_code IS NULL OR TRIM(mdms.business_entity_code) = '')
AND   legacy.distb_cntry <> '' AND gam.original_imc_type_desc <> ndt_ddsttyp.dist_type_desc; -- 5427897
...
FROM curated_integration.global_account_master gam
  ...
  JOIN atomic.wwt01020_aff_mst am_valid_aff
		ON am_valid_aff.aff_id = CASE WHEN TRIM(legacy.intgrt_ctl_aff) = '' THEN 0	ELSE legacy.intgrt_ctl_aff::SMALLINT END
	LEFT JOIN atomic.dwv41141_account_dtl mdms ON CONCAT (LPAD(legacy.intgrt_ctl_aff,'3','0'),legacy.distb_nbr) = CONCAT (LPAD(mdms.affiliate_code,'3','0'),mdms.abo_no)
	LEFT JOIN atomic_legacy.dwt41047_ddsttyp ndt_ddsttyp ON legacy.imc_type_code = ndt_ddsttyp.dist_type AND legacy.distb_cntry = ndt_ddsttyp.intgrt_cntry_cd
WHERE (mdms.affiliate_code IS NULL OR TRIM(mdms.business_entity_code) = '')
AND   legacy.distb_cntry <> '' 
...
FROM curated_integration.global_account_master gam
  ...
  JOIN atomic.wwt01020_aff_mst am_valid_aff
	...
	JOIN atomic_legacy.dwt41046_dbusnat ndt_dbusnat ON legacy.business_nature_code = ndt_dbusnat.bus_nat_cd AND legacy.distb_cntry = ndt_dbusnat.intgrt_cntry_cd
WHERE (mdms.affiliate_code IS NULL OR TRIM(mdms.business_entity_code) = '')
AND   legacy.distb_cntry <> '' AND ndt_dbusnat.bus_nat_cd IS NOT NULL AND TRIM(ndt_dbusnat.bus_nat_desc) <> ''
...
FROM curated_integration.global_account_master gam
  ...
  JOIN atomic.wwt01020_aff_mst am_valid_aff
	...
	LEFT JOIN atomic.dwv41141_account_dtl mdms ON CONCAT (LPAD(legacy.intgrt_ctl_aff,'3','0'),legacy.distb_nbr) = CONCAT (LPAD(mdms.affiliate_code,'3','0'),mdms.abo_no)
WHERE (CONCAT(mdms.affiliate_code,mdms.abo_no) IS NULL OR TRIM(mdms.business_entity_code) = '')
AND legacy.distb_cntry <> '' AND gam.original_application_date = 
CASE
...
FROM curated_integration.global_account_master gam
  ...
  JOIN atomic_legacy.dwt41150_dist_type_legacy_lkp ndt_dist_type_legacy_lkp
    ON ndt_dist_type_legacy_lkp.aff_no = case
  					when trim(legacy.intgrt_ctl_aff) = '' then 0
  					else legacy.intgrt_ctl_aff::SMALLINT
  				  end
    AND legacy.imc_type_code = ndt_dist_type_legacy_lkp.local_imc_type_code
    AND legacy.business_nature_code = ndt_dist_type_legacy_lkp.local_business_nature_code
WHERE (mdms.affiliate_code IS NULL OR TRIM(mdms.business_entity_code) = '')
AND   legacy.distb_cntry <> ''  --89677385
...
FROM curated_integration.global_account_master gam
  ...
  JOIN atomic.wwt01020_aff_mst am_valid_aff
		ON am_valid_aff.aff_id = CASE WHEN TRIM(legacy.intgrt_ctl_aff) = '' THEN 0	ELSE legacy.intgrt_ctl_aff::SMALLINT END
	JOIN public.dwt41036_ddatbat_renewal_qa bat ON bat.intgrt_ctl_aff = LPAD(legacy.intgrt_ctl_aff,'3','0') AND bat.distb_nbr = legacy.distb_nbr
  LEFT JOIN atomic.dwv41141_account_dtl mdms ON CONCAT (LPAD(legacy.intgrt_ctl_aff,'3','0'),legacy.distb_nbr) = CONCAT (LPAD(mdms.affiliate_code,'3','0'),mdms.abo_no)
WHERE (CONCAT(mdms.affiliate_code,mdms.abo_no) IS NULL OR TRIM(mdms.business_entity_code) = '')
AND legacy.distb_cntry <> ''  --29707708
...
FROM curated_integration.global_account_master gam
  ...
  JOIN atomic.wwt01020_aff_mst am_valid_aff
		ON am_valid_aff.aff_id = CASE WHEN TRIM(legacy.intgrt_ctl_aff) = '' THEN 0	ELSE legacy.intgrt_ctl_aff::SMALLINT END
	LEFT JOIN public.dwt41036_ddatbat_renewal_qa bat ON bat.intgrt_ctl_aff = LPAD(legacy.intgrt_ctl_aff,'3','0') AND bat.distb_nbr = legacy.distb_nbr
  LEFT JOIN atomic.dwv41141_account_dtl mdms ON CONCAT (LPAD(legacy.intgrt_ctl_aff,'3','0'),legacy.distb_nbr) = CONCAT (LPAD(mdms.affiliate_code,'3','0'),mdms.abo_no)
WHERE (CONCAT(mdms.affiliate_code,mdms.abo_no) IS NULL OR TRIM(mdms.business_entity_code) = '')
AND legacy.distb_cntry <> ''
...
FROM curated_integration.global_account_master gam
  ...
  LEFT JOIN atomic_legacy.dwt41042_distb_dtl legacy ON atlas.imc_number = CONCAT(LPAD( legacy.intgrt_ctl_aff,'3','0'),legacy.distb_nbr)
  LEFT JOIN atomic.dwv41141_account_dtl mdms ON atlas.imc_number = CONCAT(LPAD( mdms.affiliate_code,'3','0'),mdms.abo_no)
WHERE (mdms.affiliate_code IS NULL OR TRIM(mdms.business_entity_code) = '')
AND   (legacy.intgrt_ctl_aff IS NULL OR TRIM(legacy.distb_cntry) = '')
AND   TRIM(atlas.imc_type) <> 'INTERCOMPANY'
...
FROM curated_integration.global_account_master gam
  ...
  JOIN atomic.wwt01020_aff_mst am_valid_aff
	...
	LEFT JOIN atomic.dwv41141_account_dtl mdms ON CONCAT (LPAD(legacy.intgrt_ctl_aff,'3','0'),legacy.distb_nbr) = CONCAT (LPAD(mdms.affiliate_code,'3','0'),mdms.abo_no)
WHERE (CONCAT(mdms.affiliate_code,mdms.abo_no) IS NULL OR mdms.business_entity_code = '')
AND   legacy.distb_cntry <> '' AND legacy.auto_renew = 'Y' AND gam.is_auto_renewal_subscribed IS TRUE; -- 1900819
...
FROM curated_integration.global_account_master gam
  ...
  JOIN atomic.wwt01020_aff_mst am_valid_aff
	...
	LEFT JOIN atomic.dwv41141_account_dtl mdms ON CONCAT (LPAD(legacy.intgrt_ctl_aff,'3','0'),legacy.distb_nbr) = CONCAT (LPAD(mdms.affiliate_code,'3','0'),mdms.abo_no)
WHERE (CONCAT(mdms.affiliate_code,mdms.abo_no) IS NULL OR mdms.business_entity_code = '')
AND   legacy.distb_cntry <> '' AND legacy.auto_renew <> 'Y' AND gam.is_auto_renewal_subscribed IS FALSE; -- 80217926
